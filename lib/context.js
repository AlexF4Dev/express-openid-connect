...
async function refresh({ tokenEndpointParams } = {}) {
  ...
  if (oldTokenSet.dpopJwk) {
    extras.DPoP = JSON.parse(Buffer.from(oldTokenSet.dpopJwk, 'base64'));
  }
...
class RequestContext {
  ...
  isDpop() {
    return !!tokenSet.call(this).dpopJwk;
  }
  ...
  async dpopProof(url, method) {
    let { config, next } = weakRef(this);
    next = once(next);
    try {
      const { dpopJwk, access_token } = tokenSet.call(this);
      let DPoP;

      if (!access_token) {
        throw new Error('No DPoP access token available');
      }

      if (dpopJwk) {
        DPoP = JSON.parse(Buffer.from(dpopJwk, 'base64'));
      } else {
        throw new Error('DPoP is not enabled');
      }

      const { client } = await getClient(config);
      return client.dpopProof(
        {
          htu: url,
          htm: method,
        },
  ...
  async fetchUserInfo() {
    ...
    const tokens = tokenSet.call(this);

    let extras = {};

    if (tokens && tokens.dpopJwk) {
      extras.DPoP = JSON.parse(Buffer.from(tokens.dpopJwk, 'base64'));
    }

    return client.userinfo(tokens, extras);
  }
}
    ...
    try {
      ...
      try {
        ...
        if (config.dpop) {
          const createJWK = () => {
            let jwk = null;
            if (config.dpopTokenSigningAlg === 'RS256') {
              jwk = JWK.generateSync('RSA', 2048, {
                use: 'sig',
              });
            } else if (config.dpopTokenSigningAlg === 'ES256') {
              jwk = JWK.generateSync('EC', 'P-256', {
                use: 'sig',
                alg: 'ES256',
              });
            } else {
              throw new Error('Unsupported DPoP token signing algorithm');
            }

            return jwk;
          };

          dpopJwk.DPoP = createJWK().toJWK(true);
        }
        ...
        if (dpopJwk.DPoP) {
          tokenSet.dpopJwk = Buffer.from(JSON.stringify(dpopJwk.DPoP)).toString(
            'base64'
          );
        }
      } catch (error) {
